...
const grafana_client_1 = require("./grafana-client");
...
class DashboardMigrator {
    constructor(config, logger) {
        ...
        this.grafanaClient = new grafana_client_1.GrafanaClient(config.grafana, logger);
        this.queryTransformer = new query_transformer_1.QueryTransformer(logger);
    }
    async testConnection() {
        return await this.grafanaClient.testConnection();
    }
    async migrateDashboards(dryRun = false) {
    ...
    async migrateDashboard(originalDashboard, dryRun) {
        ...
        try {
            ...
            if (!dryRun && this.config.grafana) {
                try {
                    const created = await this.grafanaClient.createDashboard(newDashboard, originalDashboard.meta.folderId, false);
    ...
    deduplicateDashboards(dashboards) {
        const seen = new Set();
    ...
    async generateMigrationReport(summary) {
        const lines = [];
...
//# sourceMappingURL=dashboard-migrator.js.map