...
class Migrator {
    constructor(config, logger) {
        ...
        this.checkpointManager = new checkpoint_1.CheckpointManager(config.migration.checkpointPath, logger);
        // Generate migration ID
        const migrationId = this.checkpointManager.generateMigrationId(config.source.host, config.source.bucket, config.destination.host, config.destination.database);
        this.progress = {
            migrationId,
            totalRecords: 0,
            migratedRecords: 0,
    ...
    async migrate(startTime, endTime) {
        try {
    ...
    async deleteCheckpoint(migrationId) {
