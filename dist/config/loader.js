    ...
    static merge(...configs) {
        ...
        for (const config of configs) {
            ...
            if (config.destination)
                merged.destination = { ...merged.destination, ...config.destination };
            if (config.grafana)
                merged.grafana = { ...merged.grafana, ...config.grafana };
            if (config.migration)
                merged.migration = { ...merged.migration, ...config.migration };
        }
        return merged;
    }
    static validate(config) {
        ...
        if (!config.destination) {
            ...
            if (!config.destination.database)
                errors.push('Destination database is required');
        }
        return errors;
    }
}
